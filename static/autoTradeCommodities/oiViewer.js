...
function showOiViewer() {
    ...
    showPopUpWindow('oi-viewer-scanner', html, "COMMODITIES OI VIEWER", 950, 550);
...
async function getDataFormatted(token) {
...
async function showOiAnalyzer() {
    ...
    for (let i = 0; i < MCX_FUTURES.length; i++) {
        let name = MCX_FUTURES[i].name;
            let obj = {}
            obj['TRADINGSYMBOL'] = name;
            obj['TOKEN'] = MCX_FUTURES[i].instrument_token;
            let data = await getDataFormatted(MCX_FUTURES[i].instrument_token)
            obj['LTP'] = data[data.length - 1]['close'];
            scripts.push(obj)
    }
    ...
    trendingScannerTable = jQ('#trending-stock-list-table').DataTable({
        ...
        "columns": [

            {
                ...
                render: function (data, type, row, meta) {
                    let html = ''
                    html += '<a target="_blank" href="https://kite.zerodha.com/chart/ext/tvc/' + 'NSE' + '/' + data + '/' + row['TOKEN'] + '"> '
                    html += data;
                    html += '</a>'

                    html += '<span style="font-size:xx-small;position:absolute;right:0" data-token="' + row['TOKEN'] + '" data-price="' + row['LTP'] + '" data-index="' + 0 + '" data-trend="' + row['TREND'] + '" data-name="' + row['TRADINGSYMBOL'] + '" class="bg-info-color show-chart">'
                    html += 'Chart'
                    html += '</span>'

                    html += '<span style="font-size:xx-small;position:absolute;right:2rem;"data-token="' + row['TOKEN'] + '" data-price="' + row['LTP'] + '" data-index="' + 0 + '" data-trend="' + row['TREND'] + '" data-name="' + row['TRADINGSYMBOL'] + '" class="bg-info-color show-option-change">'
                    html += 'OI'
                    html += '</span>'
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            for (var i in aData) {
                ...
                jQ('td:eq(' + 12 + ')', nRow).addClass('strike-class');
...
async function callAnalyseTrend() {
    ...
    for (let i = 0; i < scriptsCount; i++) {
        ...
        try {
            ...
            if (name != 'GIFT NIFTY') {
                let oiData = await showTrendingOI(name,trendingStocks[i]['LTP'])
...
async function showTrendingOI(instrument,ltp) {
...
async function showOITrendingDetails(strikeData, selectedStrike) {
    ...
    jQ.each(strikeMap, function (index, item) {
        try {
            ...
            let obj = {}
            obj['OI_CE'] = parseFloat(OI_CE / 100).toFixed(1)
            obj['CHG_OI_CE'] = parseFloat((OI_CE - PREV_OI_CE) / 100).toFixed(1)
            obj['STRIKE'] = index
            obj['OI_PE'] = parseFloat(OI_PE / 100).toFixed(1)
            obj['CHG_OI_PE'] = parseFloat((OI_PE - PREV_OI_PE) / 100).toFixed(1)
